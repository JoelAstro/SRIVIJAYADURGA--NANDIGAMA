...
const HeroSection: React.FC = () => {
  const { getAverageRating, ratings, cmsSettings } = useApp();
  
  const navigateTo = (hash: string) => {
    window.location.hash = hash;
  };

  const heroBg = cmsSettings?.heroBgImage || '/hero_background.png';
        ...
        <div className="w-[90%] sm:w-full sm:max-w-[700px] space-y-8 flex flex-col items-center sm:items-start text-center sm:text-left">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-saffron/10 border border-saffron/30 text-saffron text-[10px] font-bold uppercase tracking-widest backdrop-blur-sm">
              ...
              {cmsSettings?.restaurantTagline || '❄️ Fully A/C Fine Dining'}
            </div>
          ...
          <div className="space-y-4">
            ...
            >
              {cmsSettings?.heroDescription || 'Welcome to Sri Vijaya Durga Family AC Restaurant. Savor authentic culinary delicacies, signature Hyderabadi Biryanis, claypot tandoori platters, and premium catering.'}
            </motion.p>
          </div>
          ...
          >
            <button 
              onClick={() => navigateTo(cmsSettings?.primaryButtonUrl || '#booking')}
            ...
            >
              {cmsSettings?.primaryButtonText || 'Reserve a Table'} <ArrowRight className="w-4 h-4" />
            </button>
            <button 
              onClick={() => navigateTo(cmsSettings?.secondaryButtonUrl || '#menu')}
            ...
            >
              {cmsSettings?.secondaryButtonText || 'Explore Digital Menu'} <BookOpen className="w-4 h-4" />
            </button>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-center sm:justify-start gap-4">
              ...
              <div className="text-center sm:text-left">
                ...
                <p className="text-xs sm:text-sm text-white font-extrabold font-logo drop-shadow-md">{cmsSettings?.hoursMonday || '11:00 AM - 11:00 PM'}</p>
              </div>
            </div>
            <a 
              href={`tel:+91${cmsSettings?.primaryPhone || '9966315544'}`} 
              ...
              aria-label="Call Sri Vijaya Durga Restaurant for Orders and Catering"
              title="Call Now"
            >
              ...
              <div className="text-center sm:text-left">
                ...
                <p className="text-xs sm:text-sm text-white font-extrabold font-logo group-hover:text-saffron transition-colors duration-300 drop-shadow-md">{cmsSettings?.primaryPhone || '9966315544'}</p>
              </div>
            </a>
          </motion.div>
