...
const ContactSection: React.FC = () => {
  ...
  const mapLink = cmsSettings?.googleMapsUrl || "https://maps.app.goo.gl/qAypkmgzgzxfD6ND8?g_st=aw";
  const address = cmsSettings?.contactAddress || "Beside TTD Kalyana Mandapam, Vijaya talkies Road, Nandigama";
  const primaryPhone = cmsSettings?.primaryPhone || "9966315544";
  const secondaryPhone = cmsSettings?.secondaryPhone || "9030121200";
  const whatsappNum = cmsSettings?.whatsappNumber || "9030121200";
  const mapsImage = cmsSettings?.googleMapsCardImage || "https://images.unsplash.com/photo-1524661135-423995f22d0b?auto=format&fit=crop&q=80&w=600";
    ...
    <div className="w-full bg-white dark:bg-bg-dark border border-maroon/10 dark:border-saffron/10 rounded-3xl p-6 sm:p-10 shadow-lg relative glass overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
        ...
        <div className="space-y-6 flex flex-col justify-center">
          <div className="space-y-2">
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400 leading-relaxed">
              We look forward to hosting you at {cmsSettings?.restaurantName || 'Sri Vijaya Durga Restaurant'}. Reach out for home delivery, custom catering, or reservations.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-4 items-start">
              ...
              <div className="space-y-1">
                ...
                {cmsSettings?.contactLandmark && <p className="text-[10px] text-neutral-400">Landmark: {cmsSettings.contactLandmark}</p>}
              </div>
            </div>
            ...
            <div className="flex gap-4 items-start">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs font-bold">
                  <a href={`tel:${primaryPhone}`} className="hover:underline">Cell: {primaryPhone}</a><br />
                  {secondaryPhone && <a href={`tel:${secondaryPhone}`} className="hover:underline">Phone: {secondaryPhone}</a>}
                </p>
              </div>
            </div>
        ...
        <div className="h-64 sm:h-auto min-h-[260px] bg-neutral-100 dark:bg-neutral-800/40 rounded-2xl border border-neutral-200 dark:border-neutral-700 relative overflow-hidden flex flex-col justify-end p-6 select-none shadow-inner">
          <div className="absolute inset-0 z-0 bg-cover bg-center" style={{ backgroundImage: `url('${mapsImage}')` }}></div>
